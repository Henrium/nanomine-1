...
def runmodel(request):
	if request.user.is_authenticated():
		...
		# Link of Apache page
		link2result = 'http://puma.mech.northwestern.edu/nanomine/RECON/'+timestamp+'_'+str(int(count[0]))
		...
		return render_to_response("RECONRun.html", {'jobid':JOBID}, context_instance=RequestContext(request))
	else:
		return redirect('/login')
	...
	if request.user.is_authenticated():
		...
		if len(request.POST) != 0:
			...
			if FINISH == 1:		
				...
				cpcp = 'cp '+WorkingDir+'/'+FILE+'_results/* '+ApacheCaseDir
				print cpcp
				os.system(cpcp)
				HTTPROOT = 'http://puma.mech.northwestern.edu/nanomine/RECON/'+str(jobID)
