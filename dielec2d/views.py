...
def validate(request):
    ...
    if request.user.is_authenticated():
        ...
        if len(request.POST) > 0:
            ...
            # init empty strings for template 
            ...
            if request.POST['method'] == 'one':
                ...
                # determine url to images for the materials 
                if graft == 'bare':
                    BINIMGFILE = \
                ...
                # write computed results to params
    ...
    if request.user.is_authenticated():
        ...
        if not IN_QUEUE:  # Run model if a port can be assigned
            ...
            if METHOD == '1':  
                ...
                str_PATH = \
                    "addpath('/usr/local/comsol52a/multiphysics/mli', '/usr/local/comsol52a/multiphysics/mli/startup');"
                ...
                str_matlab_run_model = \
                    ...
                    + str(PORT) + ');exit" > ~/ONR/dielec2d_web/log/MATLABRUN_' \
                    + run_id + '.log &'
                ...
                with open(LocalDataDir+'/FH.runconfig', 'r') as f:
                    FH = float(f.read())
                with open(LocalDataDir+'/rc.runconfig', 'r') as f:
                    rc = f.read()
                with open(LocalDataDir+'/rd.runconfig', 'r') as f:
                    rd = f.read()
                ...
                ssh_run_method_2 = \
                    ...
                    + ');exit" > ~/ONR/dielec2d_web/log/MATLABRUN_' + run_id + '.log &'
                print 'Run MATLAB shell command:'
