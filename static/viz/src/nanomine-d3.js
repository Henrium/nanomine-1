    ...
    function getComposite(uri) {
        if (!cache[uri]) {
            cache[uri] = {
                ...
                "http://nanomine.tw.rpi.edu/ns/Filler": {},
                "http://nanomine.tw.rpi.edu/ns/Matrix" : {},
            ...
            .then(function(data) {
                return $q(function( resolve, reject) {
                    ...
                    data.data.results.bindings.forEach(function(row) {
                        if (row.materialType.value == "http://nanomine.northwestern.edu:8001/blazegraph/undefined") {
                        ...
                        //console.log("Printing composite");
                        //console.log(composite);
                        ['Filler','SurfaceTreatment','Matrix'].forEach(function(t) {
                    ...
                    // Only return groups with data points in them.
...
} group by ?type ?materialType ?unit order by (?type)';
            ...
            .then(function(data) {
                return $q(function( resolve, reject) {
                    ...
                    [["http://nanomine.tw.rpi.edu/ns/PolymerNanocomposite", "Polymer Nanocomposite"],
                     ["http://nanomine.tw.rpi.edu/ns/Filler", "Filler"],
                     ["http://nanomine.tw.rpi.edu/ns/Matrix", "Matrix"],
            ...
            scope.$watchCollection("config",function() {
                ...
                loadData(scope.config).then(function(data) {
                    ...
                    if (data.config.x) {
                        ...
                        scope.chart.chart.xAxis.tickFormat = scope.chart.chart.xScale.tickFormat(1,d3.format(",d"));
                    }
                    if (data.config.y) {
                        scope.chart.chart.yAxis.axisLabel = data.config.y.label;
                        scope.chart.chart.yAxis.tickFormat = scope.chart.chart.yScale.tickFormat(1,d3.format(",d"));
                    }
                });
            });
